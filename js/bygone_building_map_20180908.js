		...
		if (source.getState() == 'ready') {
			...
			$.ajax('../files/Ottawa_Bygone_Buildings.kml').done(function(data) {
				kmlLayer.getSource().forEachFeature(function(feature) {
					...
					// Get the style from the styleURL in properties
					// Ex: For the following URL, edu_map is extracted
					//		http://127.0.0.1:4000/geomatics/files/Ottawa_Bygone_Buildings.kml#edu_map
					//var tmpstyle = styles[properties['styleUrl'].replace("#", "")];
					style_map = properties['styleUrl'].split("#")[1]
					
					// Set the tmpstyle to one of the styles based on the style_map
					var tmpstyle = styles[style_map];
					
					// Set the feature style to the tmpstyle or undefined
