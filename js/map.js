...
function Map() {
	...
	this.refreshLayers = function () {
		...
		*/
		...
		var total_layers = this.map.getLayers().getLength();
		for (i = 0; i < 2; i++) {
			var lyr_name = this.map.getLayers().item(i).get('name')
			if (lyr_name == check_rad) {
				this.map.getLayers().item(i).setVisible(true);
			} else {
				this.map.getLayers().item(i).setVisible(false);
			}
		}
	};
	
	this.setLayers = function (lyrList, hlightList) {
		return null;
	};
}
